			...
			if (App->gui->UiUnderMouse() == nullptr && App->pathfinding->IsWalkable(Tile, 2)) {
				...
				if (build) {
					if (EnoughResources(ToCreate)) {
						...
						NotEnaughResourcesText = App->gui->AddText(10, 500, "Not enough resources", App->font->Small, { 152,30,30,0 }, 16, false, false, Panel);
						build = false;
...
void EntityManager::ui_callback(UiElement* element) {
	if (element->type==UiTypes::EButton) {
		if (((UiEntityButton*)element)->entitytype == EntityType::TypeBuilding) {
			...
			App->gui->AddText(60, 16, "BUILD", App->font->Small, { 65,175,94 }, 42, false, false, BuildButton);
			CopperIcon = App->gui->AddImage(20, 450, { 679,501,28,29 }, false, false, Panel, nullptr);
			TitaniumIcon = App->gui->AddImage(110, 450, { 641,498,30,31 }, false, false, Panel, nullptr);
